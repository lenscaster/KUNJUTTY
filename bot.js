...
*/
                        ...
                        try {
                            ...
                            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                                ...
                                await conn.sendMessage(conn.user.jid, '*-- ERROR ONDALLO ALIYA [Ponnus] --*' + 
                                    '\n*Ponnu an error has occurred!*'+
                                    ...
                                    '\n*Aslo you can join our support group:* https://chat.whatsapp.com/IbxALnFjOlDE0X0lSf1mKe' +
                                    '\n*This message should have gone to your number (saved messages).*\n\n' +
                                    '*Error:* ```' + error + '```\n\n'
                                    , MessageType.text, {detectLinks: false}
                                );
                                if (error.message.includes('URL')) {
                                    return await conn.sendMessage(conn.user.jid, '*⚕️ ERROR ONDALLO ALIYA [Ponnus] ⚕️*' + 
                                        '\n========== ```Error Resolved!``` ==========' +
                                    ...
                                    return await conn.sendMessage(conn.user.jid, '*🙇🏻 Preshnam Nthann Ariyillalo Aliya! 🙇🏻*' +
